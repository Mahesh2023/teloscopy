...
class TestSentimentAnalysis:
    """Unit tests for the _analyze_sentiment function (VADER + multi-label themes)."""

    def test_severe_negative_sentiment(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I am drowning in despair and everything is hopeless.")
        assert result["compound"] < -0.5
        assert result["intensity"] in ("severe", "high")
        assert result["negative"] > 0.0

    def test_positive_sentiment(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I am so happy and grateful for everything in my life!")
        assert result["compound"] > 0.3
        assert result["intensity"] == "positive"
        assert result["positive"] > 0.0

    def test_neutral_sentiment(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("Hello, I want to talk about something.")
        assert result["intensity"] in ("neutral", "mild")
        assert result["primary_theme"] == "self_knowledge"

    def test_multi_label_themes(self):
        """Multiple emotions in one message should detect ALL themes."""
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I feel anxious and lonely at the same time.")
        assert "anxiety" in result["themes"]
        assert "loneliness" in result["themes"]
        assert len(result["themes"]) >= 2

    def test_theme_scores_ranking(self):
        """Primary theme should be the one with most keyword matches."""
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I keep comparing myself to others and feel inadequate.")
        assert result["primary_theme"] == "comparison"
        assert result["theme_scores"]["comparison"] >= 2

    def test_fear_theme(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I am afraid of the dark and feel scared.")
        assert "fear" in result["themes"]

    def test_anger_theme(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I am so angry and furious about what happened.")
        assert "anger" in result["themes"]

    def test_depression_theme_with_intensity(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I feel depressed, empty, and worthless.")
        assert "depression" in result["themes"]
        assert result["intensity"] in ("severe", "high")

    def test_relationship_theme(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("My marriage is falling apart and I had a breakup.")
        assert "relationship" in result["themes"]

    def test_thought_theme(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("I can't stop overthinking and my mind won't stop.")
        assert "thought" in result["themes"]

    def test_default_fallback(self):
        from teloscopy.webapp.app import _analyze_sentiment
        result = _analyze_sentiment("hello world")
        assert result["primary_theme"] == "self_knowledge"
        assert result["themes"] == []

    def test_sentiment_in_counsel_response(self, client, consent_headers):
        """The /api/psychiatry/counsel response should include sentiment data."""
        resp = client.post(
            "/api/psychiatry/counsel",
            json={"message": "I feel deeply anxious about the future."},
            headers=consent_headers,
        )
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "themes" in s
        assert "primary_theme" in s
        assert isinstance(s["themes"], list)
        assert "anxiety" in s["themes"]

    def test_intensity_aware_acknowledgment(self):
        """Severe distress should get warm prefix in template response."""
        from teloscopy.webapp.app import _build_counselling_response, _analyze_sentiment
        msg = "I feel completely worthless and hopeless and disgusted with myself."
        sentiment = _analyze_sentiment(msg)
        assert sentiment["intensity"] in ("severe", "high")
        # Run template builder directly (avoids rate limiter)
        resp = _build_counselling_response(sentiment["primary_theme"], msg, sentiment=sentiment)
        ack = resp["response"]["acknowledgment"]
        _warm_markers = ["hear you", "painful", "difficult", "here", "weight", "rush",
                         "sharing", "feeling", "valid", "matters deeply", "carrying"]
        assert any(m in ack.lower() for m in _warm_markers), f"Expected warm prefix in: {ack[:150]}"

    def test_backward_compat_match_counselling_theme(self):
        """The wrapper _match_counselling_theme should still work."""
        from teloscopy.webapp.app import _match_counselling_theme
        assert _match_counselling_theme("I am afraid of the dark") == "fear"
        assert _match_counselling_theme("I feel anxious about exams") == "anxiety"
        assert _match_counselling_theme("hello world") == "self_knowledge"
